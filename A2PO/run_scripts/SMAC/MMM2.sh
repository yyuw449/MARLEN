...
do
    ...
    set -x
    CUDA_VISIBLE_DEVICES=${gpu} python onpolicy/scripts/train/train_smac.py --env_name ${env} --algorithm_name ${algo} --experiment_name ${exp} --map_name ${map} --seed ${seed} --n_rollout_threads ${n_t} --episode_length $((3200 / $n_t)) --use_eval True --use_aim True --n_run 1 --ppo_epoch 5 --use_sequential True --loop_order ppo --seq_strategy reverse_greedy --clip_others False
done
# mappo
# --ppo_epoch 5
