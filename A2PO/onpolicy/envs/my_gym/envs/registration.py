...
from onpolicy.envs.my_gym import error, logger
...
plugin_internal_whitelist = {"ale_py.onpolicy.envs.my_gym"}
...
class EnvSpec(object):
    ...
    def make(self, **kwargs):
        ...
        if callable(self.entry_point):
            ...
            print("self.entry_point",self.entry_point)
        ...
        if env.spec.max_episode_steps is not None:
            ...
            if self.order_enforce:
                from onpolicy.envs.my_gym.wrappers.order_enforcing import OrderEnforcing
        ...
        try:
            ...
            if matching_envs:
                ...
                raise error.UnregisteredEnv(
                    "Algorithmic environment {} has been moved out of Gym. Install it via `pip install gym-algorithmic` and add `import onpolicy.envs.my_gym_algorithmic` before using it.".format(
                        id
                    )
                )
            elif env_name in toytext_envs:
                raise error.UnregisteredEnv(
                    "Toytext environment {} has been moved out of Gym. Install it via `pip install gym-legacy-toytext` and add `import onpolicy.envs.my_gym_toytext` before using it.".format(
                        id
                    )
                )
            else:
...
def load_env_plugins(entry_point="onpolicy.envs.my_gym.envs"):
