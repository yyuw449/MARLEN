...
import onpolicy.envs.my_gym
import numpy as np
from onpolicy.envs.my_gym import logger
from onpolicy.envs.my_gym import spaces
...
def _check_returned_values(
    ...
    """
    # because env inherits from onpolicy.envs.my_gym.Env, we assume that `reset()` and `step()` methods exists
    obs = env.reset()
...
def _check_spaces(env: gym.Env) -> None:
    ...
    and inherit from onpolicy.envs.my_gym.spaces.Space.
    """
    ...
    assert isinstance(env.observation_space, spaces.Space), (
        "The observation space must inherit from onpolicy.envs.my_gym.spaces" + gym_spaces
    )
    assert isinstance(env.action_space, spaces.Space), (
        "The action space must inherit from onpolicy.envs.my_gym.spaces" + gym_spaces
    )
