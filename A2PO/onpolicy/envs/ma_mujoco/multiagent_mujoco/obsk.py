            ...
            for c in cats:
                if c in _t.extra_obs:
                    ...
                    if c in ["qvel", "qpos"
                        ...
                        items = getattr(env.sim.data,
                                        c)[getattr(_t, "{}_ids".format(c))]
                        obs_lst.extend(
                            items if isinstance(items, list) else [items])
                    elif c in ["qfrc_actuator"
                               ]:  # this is a "vel position" item
                        items = getattr(env.sim.data,
                                        c)[getattr(_t,
                                                   "{}_ids".format("qvel"))]
                        obs_lst.extend(
                            items if isinstance(items, list) else [items])
                    elif c in ["cvel", "cinert",
                               "cfrc_ext"]:  # this is a "body position" item
                        if _t.bodies is not None:
                            for b in _t.bodies:
                                ...
                                if b not in body_set_dict[c]:
                                    items = getattr(env.sim.data,
                                                    c)[b].tolist()
                                    items = getattr(_t, "body_fn",
                                                    lambda _id, x: x)(b, items)
                                    obs_lst.extend(items if isinstance(
                                        items, list) else [items])
...
def get_parts_and_edges(label, partitioning):
    if label in ["half_cheetah", "HalfCheetah-v2"]:
        ...
        if partitioning == "2x3":
            ...
            parts = [(bfoot, ), (bshin, ), (bthigh, ), (ffoot, ), (fshin, ),
                     (fthigh, )]
            ...
            raise Exception(
                "UNKNOWN partitioning config: {}".format(partitioning))
        ...
        free_joint = Node(
            ...
            extra_obs={
                ...
                "cfrc_ext":
                lambda env: np.clip(env.sim.data.cfrc_ext[0:1], -1, 1)
            })
        ...
        if partitioning == "2x4":  # neighbouring legs together
            ...
            parts = [(hip1, ), (ankle1, ), (hip2, ), (ankle2, ), (hip3, ),
                     (ankle3, ), (hip4, ), (ankle4, )]
        else:
            raise Exception(
                "UNKNOWN partitioning config: {}".format(partitioning))
        ...
        thigh_joint = Node(
            ...
            extra_obs={
                "qvel":
                lambda env: np.clip(np.array([env.sim.data.qvel[-3]]), -10, 10)
            })
        leg_joint = Node(
            "leg_joint",
            -2,
            ...
            extra_obs={
                "qvel":
                lambda env: np.clip(np.array([env.sim.data.qvel[-2]]), -10, 10)
            })
        foot_joint = Node(
            "foot_joint",
            -1,
            ...
            extra_obs={
                "qvel":
                lambda env: np.clip(np.array([env.sim.data.qvel[-1]]), -10, 10)
            })
        ...
        # define Mujoco-Graph
        abdomen_y = Node("abdomen_y", -16, -16,
                         0)  # act ordering bug in env -- double check!
        ...
        edges = [
            ...
            HyperEdge(left_shoulder1, left_shoulder2, abdomen_x, abdomen_y,
                      abdomen_z),
            HyperEdge(right_shoulder1, right_shoulder2, abdomen_x, abdomen_y,
                      abdomen_z),
            HyperEdge(abdomen_x, abdomen_y, abdomen_z, left_hip_x, left_hip_y,
                      left_hip_z),
            HyperEdge(abdomen_x, abdomen_y, abdomen_z, right_hip_x,
        ...
        if partitioning == "9|8":  # 17 in total, so one action is a dummy (to be handled by pymarl)
            ...
            parts = [(left_shoulder1, left_shoulder2, abdomen_x, abdomen_y,
                      abdomen_z, right_shoulder1, right_shoulder2, right_elbow,
                      left_elbow),
                     (left_hip_x, left_hip_y, left_hip_z, right_hip_x,
                      right_hip_y, right_hip_z, right_knee, left_knee)]
            ...
            # isolate upper and lower body
            parts = [(left_shoulder1, ), (left_shoulder2, ), (abdomen_x, ),
                     (abdomen_y, ), (abdomen_z, ), (right_shoulder1, ),
                     (right_shoulder2, ), (right_elbow, ), (left_elbow, ),
        ...
        joint1 = Node("joint1",
                      ...
                      extra_obs={
                          "fingertip_dist": (lambda env: env.get_body_com(
                              "fingertip") - env.get_body_com("target")),
                          "qpos": (lambda env: np.array([
                              np.sin(env.sim.data.qpos[-3]),
        ...
        target_x = Node("target_x",
                        ...
                        -2,
                        -1,
                        extra_obs={"qvel": (lambda env: np.array([]))})
        ...
        if partitioning == "2x3":
            ...
            # isolate upper and lower body
            parts = [(foot_joint, ), (leg_joint, ), (thigh_joint, ),
                     (foot_left_joint, ), (leg_left_joint, ),
                     (thigh_left_joint, )]
        elif partitioning == "3x2":
            # isolate upper and lower body
            parts = [(
        ...
        bthigh2 = Node("bthigh2",
                       ...
                       extra_obs={
                           ...
                           "ten_length": lambda env: env.sim.data.ten_length,
                           "ten_velocity":
                           lambda env: env.sim.data.ten_velocity
                       })
        ...
        edges = [
            HyperEdge(joints[i], joints[i + 1]) for i in range(n_segs - 1)
        ]
        globals = {}
        ...
        for si in range(n_segs):
            ...
            off = -4 * (n_segs - 1 - si)
            hip1n = Node("hip1_{:d}".format(si),
                         -4 - off,
                         -4 - off,
                         2 + 4 * si,
                         bodies=[torso, front_right_leg],
                         body_fn=lambda _id, x: np.clip(x, -1, 1).tolist())
            ankle1n = Node("ankle1_{:d}".format(si),
                           -3 - off,
                           -3 - off,
                           3 + 4 * si,
                           bodies=[front_right_leg, aux1, ankle1],
                           body_fn=lambda _id, x: np.clip(x, -1, 1).tolist())
            hip2n = Node("hip2_{:d}".format(si),
                         -2 - off,
                         -2 - off,
                         0 + 4 * si,
                         bodies=[torso, back_leg],
                         body_fn=lambda _id, x: np.clip(x, -1, 1).tolist())
            ankle2n = Node("ankle2_{:d}".format(si),
                           -1 - off,
                           -1 - off,
                           1 + 4 * si,
        ...
        parts = [[
            x for sublist in joints[i * n_segs_per_agents:(i + 1) *
                                    n_segs_per_agents] for x in sublist
        ] for i in range(n_agents)]
