...
class Scenario(BaseScenario):
    def make_world(self, args=None):
        ...
        num_landmarks = 4
        self.up = np.linspace(0.1, 0.1, num=480)
        self.down = np.linspace(0.1, 0.1, num=480)
        # add agents
        ...
        for i, agent in enumerate(world.agents):
            ...
            # agent.accel = 100000000 if agent.adversary else 10000000000
    ...
    #         other_pos[i] += np.random.normal(
    #             np.zeros_like(other_pos[i]), 
    #             np.ones_like(other_pos[i]) * (self.up[self.iter_num] if self.iter == 1 else self.down[self.iter_num])
    #             )
    #     for i in range(len(other_vel)):
    #         other_vel[i] += np.random.normal(
    #             np.zeros_like(other_vel[i]), 
    #             np.ones_like(other_vel[i]) * (self.up[self.iter_num] if self.iter == 1 else self.down[self.iter_num])
    #             )
        
    #     self.iter_num += 1
    #     # other_pos += torch.normal(torch.from_numpy(np.zeros_like(other_pos)), torch.from_numpy(np.ones_like(other_pos)) * 0.1).numpy()
    #     # other_vel += torch.normal(torch.from_numpy(np.zeros_like(other_vel)), torch.from_numpy(np.ones_like(other_vel)) * 0.1).numpy()
    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            if other is agent: continue
            #noisy observation
            noisy_x_y_pos_other = [0,0]
            noisy_x_y_pos_other[0] = other.state.p_pos[0]
            noisy_x_y_pos_other[1] = other.state.p_pos[1]
            ...
            # print("org pos:",other.state.p_pos)
            # print("noisy pos:",noisy_x_y_pos_other)
            dist = np.sqrt(np.sum(np.square(noisy_x_y_pos_other - agent.state.p_pos)))
            if agent.view_radius >= 0 and dist <= agent.view_radius:
                comm.append(other.state.c)
                other_pos.append(noisy_x_y_pos_other - agent.state.p_pos)
                if not other.adversary:
                    #noisy velocity
                    noisy_x_y_vel_other = [0,0]
                    noisy_x_y_vel_other[0] = other.state.p_vel[0]
                    noisy_x_y_vel_other[1] = other.state.p_vel[1]
                    var = np.random.normal(
                    0, 
                    self.up[self.iter_num] if self.iter == 1 else self.down[self.iter_num]
                    )
                    ...
                    other_vel.append(noisy_x_y_vel_other)
                    # print("org vel:",other.state.p_vel)
                    # print("noisy vel:",noisy_x_y_vel_other)
            else:
                other_pos.append(np.array([0., 0.]))
                if not other.adversary:
                    other_vel.append(np.array([0., 0.]))
