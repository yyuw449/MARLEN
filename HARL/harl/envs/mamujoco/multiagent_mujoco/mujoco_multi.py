...
# import gym
import harl.envs.gym_custom
from harl.envs.gym_custom.spaces import Box
from harl.envs.gym_custom.wrappers import TimeLimit
import numpy as np
...
class NormalizedActions(harl.envs.gym_custom.ActionWrapper):
    def _action(self, action):
        action = (action + 1) / 2
        high = 2 * np.copy(self.action_space.high)
        low = 2 * np.copy(self.action_space.low)
...
class MujocoMulti(MultiAgentEnv):
        ...
        if self.env_version == 2:
            try:
                self.wrapped_env = NormalizedActions(
                    harl.envs.gym_custom.make(self.scenario)
                )
            except harl.envs.gym_custom.error.Error:
                self.wrapped_env = NormalizedActions(
                    TimeLimit(
    ...
    def step(self, actions):
        ...
        # print('reached 2 : ',type(self.wrapped_env))
        # print('actions shape : ',actions.shape) #-- (2,3)
