...
class AltitudeReward(BaseRewardFunction):
    ...
    def get_reward(self, task, env, agent_id):
        ...
        Returns:
            (float): reward
        """
        with open("/hdd3/marl_new/HARL/runtime_info/safe_alt.pkl", "rb") as f:
            self.safe_altitude = pickle.load(f)

        with open("/hdd3/marl_new/HARL/runtime_info/danger_alt.pkl", "rb") as f:
            self.danger_altitude = pickle.load(f)

        with open("/hdd3/marl_new/HARL/runtime_info/Kv.pkl", "rb") as f:
            self.Kv = pickle.load(f)
            
        # print('safe altitute : ',self.safe_altitude)
        # print('danger altitute  : ',self.danger_altitude)
